...
class SmokeTest extends TestCase
{
    ...
    public static function greatCircleDestination(S2LatLng $a, int|float $bearing, int|float $distance): S2LatLng {
        ...
        return new S2LatLng($lat, $lng);
    }

    public static function encodeLocation(S2LatLng $s2ll): string {
        $lat = (int)($s2ll->latDegrees() * 1000000);
        $lng = (int)($s2ll->lngDegrees() * 1000000);
        return sprintf("%08x,%08x", $lat, $lng);
    }

    public static function decodeLocation(string $loc): bool|S2LatLng {
        if (!str_contains($loc, ',')) return false;
        list ($lat, $lng) = explode(',', $loc);
        return S2LatLng::fromDegrees(hexdec($lat) / 1000000, hexdec($lng) / 1000000);
    }

    function find_some_point_with_distance(float $lat, float $lng) : never
    {
        ...
        $loc_port = S2LatLng::fromDegrees($lat, $lng);
        for ($i = 0; $i < 10; $i++) {
            $loc_1 = S2LatLng::fromDegrees($lat_p[0], $lng_p[0]);
    ...
    private static function guidToToken(string $guid): string {
    ...
    public function testA()
    {
        ...
        $from = S2LatLng::fromDegrees(55.578201, 37.912176);
        $to = S2LatLng::fromDegrees(55.578324, 37.9109);
    ...
    public function testB()
    {
        ...
        $loc = S2LatLng::fromDegrees(55.6141284375, 37.9788514375);

        $s2ll = S2CellId::fromToken(self::guidToToken('700c7c5346a246ee88eee70b200d0b33.16'))->toLatLng();
        $this->assertEquals('(-0.023004811178492, -3.06557268979)', (string)$s2ll);

        $s2ll = S2CellId::fromToken(self::guidToToken('414ab9b68fd0000000082a7300000025.6'))->toLatLng();
        $this->assertEquals('(0.97053474915648, 0.66268771618818)', (string)$s2ll);

        $loc_1 = S2LatLng::fromDegrees(55.605873, 37.970864);
        $loc_ezhio = S2LatLng::fromE6(55608152, 37972176);
